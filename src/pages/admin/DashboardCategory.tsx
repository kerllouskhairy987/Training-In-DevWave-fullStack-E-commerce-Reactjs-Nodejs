import { useCreateCategoryMutation, useDeleteCategoryMutation, useGetAllCategoriesQuery } from "@/app/features/Dashboard/dashboardSlice"
...
const DashboardCreateCategory = () => {
    ...
    // Create Category
    const [createCategory, { isLoading, data: responseDate, isSuccess, error }] = useCreateCategoryMutation()
    ...
    // -------------------------------------------------------------
    // Delete Category
    const [deleteCategory, { isLoading: isLoadingDelete, data: dateDelete, isSuccess: isSuccessDelete, error: errorDelete }]
    ...
    return (
        <div className="bg-white w-full flex justify-center min-h-[100vh]">
            <div className="w-full px-5 md:w-[500px] lg:max-w-[600px]">
                <form onSubmit={submitHandlerCreateCategory} ref={formRef}
                    className=" text-black flex flex-col gap-4 items-center py-4 w-full">

                    <h1 className="text-2xl font-bold text-center text-black">Dashboard Create Category</h1>
                    {
                ...
                <h2 className="text-black font-bold text-center my-6">All Categories</h2>
                {
                    isLoadingAllCategories ? <AllCategoriesSkeleton /> : (
                        allCategories?.categories.map((category) => (
                            <div key={category._id} className="flex flex-col md:flex-row mb-10 md:mb-0 justify-between items-start text-black gap-2 my-2">
                                <div>
                                    <h3 className="font-semibold text-lg"><span className="text-green-700">Category Name:</span> {category.name}</h3>
                                    <span className="text-sm "><span className="text-green-700">Category Description:</span> {category.description}</span>
                                </div>
                                <div className="flex gap-3 items-center">
                                    <CustomModal id={category._id} nameValue={category.name} descValue={category.description}>
